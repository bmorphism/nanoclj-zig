...
// ============================================================================
// HELPERS — lazy-seq dispatch, type names, apply-or-builtin
// ============================================================================

const LazyKind = enum { repeat, iterate, repeatedly, range, generic };

const LazyInfo = struct {
    kind: LazyKind,
    payload: []Value, // marker-specific fields (excludes the marker keyword itself)
};

fn classifyLazy(obj: *const @import("value.zig").Obj, gc: *GC) LazyInfo {
    const thunk = obj.data.lazy_seq.thunk;
    if (!thunk.isObj()) return .{ .kind = .generic, .payload = &.{} };
    const tvec = thunk.asObj();
    if (tvec.kind != .vector) return .{ .kind = .generic, .payload = &.{} };
    const payload = tvec.data.vector.items.items;
    if (payload.len < 2 or !payload[0].isKeyword()) return .{ .kind = .generic, .payload = &.{} };
    const marker = gc.getString(payload[0].asKeywordId());
    const rest_payload = payload[1..];
    if (std.mem.eql(u8, marker, "__repeat__")) return .{ .kind = .repeat, .payload = rest_payload };
    if (std.mem.eql(u8, marker, "__iterate__")) return .{ .kind = .iterate, .payload = rest_payload };
    if (std.mem.eql(u8, marker, "__repeatedly__")) return .{ .kind = .repeatedly, .payload = rest_payload };
    if (std.mem.eql(u8, marker, "__range__")) return .{ .kind = .range, .payload = rest_payload };
...
fn seqItems(arg: Value, gc: *GC) ![]Value {
    ...
    return switch (arg.asObj().kind) {
        ...
        .vector => arg.asObj().data.vector.items.items,
        else => error.TypeError,
    };
}

pub fn valueTypeName(v: Value) []const u8 {
    if (v.isNil()) return "nil";
    if (v.isBool()) return "boolean";
    if (v.isInt()) return "integer";
    if (v.isFloat()) return "float";
    if (v.isString()) return "string";
    if (v.isKeyword()) return "keyword";
    if (v.isSymbol()) return "symbol";
    if (!v.isObj()) return "unknown";
    return switch (v.asObj().kind) {
        .list => "list",
        .vector => "vector",
        .map => "map",
        .set => "set",
        .function, .bc_closure, .builtin_ref, .partial_fn => "function",
        .macro_fn => "macro",
        .atom => "atom",
        .lazy_seq => "lazy-seq",
...
fn first(args: []Value, gc: *GC, env: *Env, _: *Resources) anyerror!Value {
    ...
    if (obj.kind == .lazy_seq) {
        const info = classifyLazy(obj, gc);
        switch (info.kind) {
            .repeat => {
                const val = if (info.payload.len >= 2) info.payload[1] else info.payload[0];
                if (info.payload.len >= 2 and info.payload[0].isInt() and info.payload[0].asInt() <= 0) return Value.makeNil();
                return val;
            },
            .iterate => return info.payload[1],
            .repeatedly => return eval_mod.apply(info.payload[0], &.{}, env, gc) catch Value.makeNil(),
            .range => return info.payload[0],
            .generic => {
                const thunk = obj.data.lazy_seq.thunk;
                if (!thunk.isNil()) {
...
fn makeLazyPayload(gc: *GC, marker: Value, fields: []const Value) !Value {
    const rest_obj = try gc.allocObj(.lazy_seq);
    const rest_payload = try gc.allocObj(.vector);
    try rest_payload.data.vector.items.append(gc.allocator, marker);
    for (fields) |f| try rest_payload.data.vector.items.append(gc.allocator, f);
...
fn rest(args: []Value, gc: *GC, env: *Env, _: *Resources) anyerror!Value {
    ...
    if (obj.kind == .lazy_seq) {
        ...
        switch (info.kind) {
            .iterate => {
                const f = info.payload[0];
                var call_args = [_]Value{info.payload[1]};
                const next_val = eval_mod.apply(f, &call_args, env, gc) catch return Value.makeObj(new);
                const marker = thunk.asObj().data.vector.items.items[0];
                return makeLazyPayload(gc, marker, &.{ f, next_val });
            },
            .range => {
                const marker = thunk.asObj().data.vector.items.items[0];
                return makeLazyPayload(gc, marker, &.{
                    Value.makeInt(info.payload[0].asInt() +% info.payload[1].asInt()),
                    info.payload[1],
                });
            },
            .repeat => {
                if (info.payload.len >= 2 and info.payload[0].isInt()) {
                    if (info.payload[0].asInt() <= 1) return Value.makeObj(new);
                    const marker = thunk.asObj().data.vector.items.items[0];
                    return makeLazyPayload(gc, marker, &.{
                        Value.makeInt(info.payload[0].asInt() - 1),
                        info.payload[1],
                    });
                }
                return args[0]; // infinite repeat
            },
            .repeatedly => return args[0],
            .generic => {
                if (!thunk.isNil()) {
...
fn takeFn(args: []Value, gc: *GC, env: *Env, _: *Resources) anyerror!Value {
    ...
    if (obj.kind == .lazy_seq) {
        ...
        switch (info.kind) {
            .repeat => {
                const val = if (info.payload.len >= 2) info.payload[1] else info.payload[0];
                const limit = if (info.payload.len >= 2 and info.payload[0].isInt())
                    @min(n, @as(usize, @intCast(@max(@as(i48, 0), info.payload[0].asInt()))))
                else
                    n;
                for (0..limit) |_| try result.data.list.items.append(gc.allocator, val);
            },
            .iterate => {
                var current = info.payload[1];
                for (0..n) |_| {
                    try result.data.list.items.append(gc.allocator, current);
                    var call_args = [_]Value{current};
                    current = eval_mod.apply(info.payload[0], &call_args, env, gc) catch break;
                }
            },
            .repeatedly => {
                for (0..n) |_| {
                    const val = eval_mod.apply(info.payload[0], &.{}, env, gc) catch break;
                    try result.data.list.items.append(gc.allocator, val);
                }
            },
            .range => {
                var current = info.payload[0].asInt();
                const step = info.payload[1].asInt();
                for (0..n) |_| {
...
fn jepsenNemesisFn(args: []Value, gc: *GC, _: *Env, _: *Resources) anyerror!Value {
    ...
    const kind: jepsen.NemesisKind = if (args[0].isKeyword()) blk: {
        ...
        if (std.mem.eql(u8, name, "none")) break :blk .none else if (std.mem.eql(u8, name, "trit-corrupt")) break :blk .trit_corrupt else if (std.mem.eql(u8, name, "trit-duplicate")) break :blk .trit_duplicate else if (std.mem.eql(u8, name, "version-rewind")) break :blk .version_rewind else if (std.mem.eql(u8, name, "eval-drop")) break :blk .eval_drop else if (std.mem.eql(u8, name, "causal-invert")) break :blk .causal_invert else break :blk .none;
...
fn dissocFn(args: []Value, gc: *GC, _: *Env, _: *Resources) anyerror!Value {
    ...
    for (keys, vals) |k, v| {
        var skip = false;
        for (args[1..]) |dk| {
            if (semantics.structuralEq(k, dk, gc)) {
                skip = true;
                break;
            }
        }
...
fn distinctFn(args: []Value, gc: *GC, _: *Env, res: *Resources) anyerror!Value {
    ...
    for (items) |item| {
        ...
        for (obj.data.vector.items.items) |existing| {
            if (semantics.structuralEq(item, existing, gc)) {
                dup = true;
                break;
            }
        }
...
// seqItems defined in helpers section above
...
fn reMatchFn(args: []Value, gc: *GC, _: *Env, _: *Resources) anyerror!Value {
    ...
    const engine: pat_mod.Engine = if (std.mem.eql(u8, engine_name, "thompson")) .thompson else if (std.mem.eql(u8, engine_name, "derivative")) .derivative else if (std.mem.eql(u8, engine_name, "backtrack")) .backtrack else return error.TypeError;
...
fn pegMatchFn(args: []Value, gc: *GC, _: *Env, _: *Resources) anyerror!Value {
    ...
    const pat = gc.getString(args[1].asStringId());
    const input = gc.getString(args[2].asStringId());
    const engine: pat_mod.Engine = if (std.mem.eql(u8, engine_name, "recursive")) .peg_rd else if (std.mem.eql(u8, engine_name, "packrat")) .peg_packrat else if (std.mem.eql(u8, engine_name, "vm")) .peg_vm else return error.TypeError;
    return matchResultToValue(pat_mod.matchWith(engine, pat, input), gc);
}
...
fn disjFn(args: []Value, gc: *GC, _: *Env, res: *Resources) anyerror!Value {
    ...
    for (items) |item| {
        ...
        for (args[1..]) |to_remove| {
            if (semantics.structuralEq(item, to_remove, gc)) {
                keep = false;
                break;
            }
        }
...
fn longFn(args: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    return intFn(args, undefined, undefined, undefined);
}
...
fn byteFn(args: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    return charFn(args, undefined, undefined, undefined);
}
...
fn anyP(args: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    _ = args;
    return Value.makeBool(true);
}
...
fn ifnP(args: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    ...
    if (!args[0].isObj()) return Value.makeBool(false);
    return Value.makeBool(switch (args[0].asObj().kind) {
        .function, .bc_closure, .builtin_ref, .partial_fn => true,
        else => false,
    });
}
...
fn specialSymbolP(_: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    return Value.makeBool(false);
}
fn varP(_: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    return Value.makeBool(false);
}
...
fn decimalP(_: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    return Value.makeBool(false);
}
fn uuidP(_: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    return Value.makeBool(false);
}
...
fn sortedP(_: []Value, _: *GC, _: *Env, _: *Resources) anyerror!Value {
    return Value.makeBool(false);
}
fn nfirstFn(args: []Value, gc: *GC, env: *Env, _: *Resources) anyerror!Value {
    var a = [_]Value{args[0]};
    const n = try nextFn(&a, gc, env, undefined);
    if (n.isNil()) return Value.makeNil();
    var b = [_]Value{n};
    return first(&b, gc, env, undefined);
}
fn nnextFn(args: []Value, gc: *GC, env: *Env, _: *Resources) anyerror!Value {
    ...
    if (n.isNil()) return Value.makeNil();
    var b = [_]Value{n};
    return nextFn(&b, gc, env, undefined);
}
fn nthnextFn(args: []Value, gc: *GC, env: *Env, _: *Resources) anyerror!Value {
    ...
    while (n > 0) : (n -= 1) {
        var a = [_]Value{cur};
        cur = try nextFn(&a, gc, env, undefined);
        if (cur.isNil()) return Value.makeNil();
    }
    return cur;
}
